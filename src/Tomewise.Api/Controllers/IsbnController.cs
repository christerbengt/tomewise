...
public class IsbnController(
    ...
    LibrisLookupService librisLookupService,
    GoogleBooksLookupService googleBooksLookupService) : ControllerBase
{
    [HttpGet("{isbn}")]
    public async Task<IActionResult> Lookup(string isbn)
    {
        ...
        var tasks = await Task.WhenAll(
            ...
            SafeLookup(() => googleBooksLookupService.LookupAsync(cleaned))
        );

        var result = tasks[0] ?? tasks[1] ?? tasks[2];
